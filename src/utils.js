...
// Для хранения и расчётов: гасит двоичный шум (7.699999809 → 7.7), но не теряет
// введённые десятичные. Округляем только при отображении (fmt0/fmt1).
export const round3 = (n) => Math.round(n * 1000) / 1000;

// Калории из БЖУ: 4/9/4 ккал на грамм; считаем точно, округляет только вывод
export function kcalFromMacros(protein, fat, carbs) {
  return round3((protein ?? 0) * 4 + (fat ?? 0) * 9 + (carbs ?? 0) * 4);
}
